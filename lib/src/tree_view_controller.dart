...
class TreeViewController extends TreeViewControllerBase with ChangeNotifier {
  ...
  void collapseAll() {
    rootNode.renderChildren.forEach(super.collapseNode);
    notifyListeners();
  }
...
class TreeViewControllerBase {
  ...
  void expandNode(TreeNode node) {
    ...
    if (node.hasChildren) {
      ...
      for (var child in node.renderChildren) {
  ...
  void expandSubtree(TreeNode node) {
    expandNode(node);
    node.renderChildren.forEach(expandSubtree);
  }
  ...
  void _populateInitialNodes() {
    for (var child in rootNode.renderChildren) {
